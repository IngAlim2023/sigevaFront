  ...
  const generarReportePDF = () => {
    try {
      ...
      if (ganador) {
        ...
        // Tabla del ganador
        const ganadorTableY = yPosition;
        const ganadorRowHeight = 12;
        const ganadorColWidths = [60, 110];
        
        const ganadorData = [
          ['Candidato:', `${ganador.nombre} ${ganador.apellido}`],
          ['Votos obtenidos:', `${ganador.votos} votos`],
          ['Porcentaje:', `${ganador.porcentaje.toFixed(2)}%`]
        ];

        doc.setFontSize(10);
        ganadorData.forEach((row, index) => {
          const currentY = ganadorTableY + (index * ganadorRowHeight);
          
          // Fondo verde claro para el ganador
          doc.setFillColor(220, 255, 220);
          
          // Bordes de la fila
          doc.setLineWidth(0.3);
          doc.rect(margin, currentY, ganadorColWidths[0], ganadorRowHeight, 'FD');
          doc.rect(margin + ganadorColWidths[0], currentY, ganadorColWidths[1], ganadorRowHeight, 'FD');
          ...
          doc.text(row[0], margin + 3, currentY + 8);
          doc.setFont('helvetica', 'normal');
          doc.text(row[1], margin + ganadorColWidths[0] + 3, currentY + 8);
        });
        
        yPosition = ganadorTableY + (ganadorData.length * ganadorRowHeight) + 15;
      }
      ...
      if (eleccion.candidatos && Array.isArray(eleccion.candidatos) && eleccion.candidatos.length > 0) {
        ...
        const candidateColWidths = [70, 30, 35, 45]; // Ajustado para mejor distribución
        
        // Verificar si necesitamos nueva página
        const estimatedTableHeight = (eleccion.candidatos.length + 1) * candidateRowHeight;
        ...
        headers.forEach((header, index) => {
          ...
          doc.setTextColor(0, 0, 0);
          doc.text(header, currentX + 2, candidateTableY + 10);
        ...
        candidatosOrdenados.forEach((candidato, index) => {
          ...
          doc.text(nombreTruncado, margin + 2, yPosition + 11);
          doc.text(candidato.votos.toString(), margin + candidateColWidths[0] + 8, yPosition + 11);
          doc.text(`${candidato.porcentaje.toFixed(1)}%`, margin + candidateColWidths[0] + candidateColWidths[1] + 5, yPosition + 11);
          doc.text(esGanador ? 'GANADOR' : '-', margin + candidateColWidths[0] + candidateColWidths[1] + candidateColWidths[2] + 2, yPosition + 11);
